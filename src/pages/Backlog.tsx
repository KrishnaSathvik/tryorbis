                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium truncate">{item.idea_name}</p>
                          <p className="text-xs text-muted-foreground">{new Date(item.created_at).toLocaleDateString()}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {(item.demand_score ?? item.overall_score) != null && (
                          <span className="text-xs text-muted-foreground">{item.demand_score ?? item.overall_score}/100</span>
                        )}
                        ...
                        <Select value={item.status} onValueChange={(v) => handleStatusChange(item.id, v as BacklogStatus)}>
                          <SelectTrigger className="w-[110px] h-8">
                            <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${statusColors[item.status as BacklogStatus] || ''}`}>{item.status}</span>
                          </SelectTrigger>
                          <SelectContent>
                            {allStatuses.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                          </SelectContent>
                    ...
                    <CollapsibleContent>
                      <div className="mt-3 pt-3 border-t space-y-3">
                        <div className="flex gap-2">
                          <Input placeholder="Add a note..." value={noteInputs[item.id] || ""} onChange={e => setNoteInputs(prev => ({ ...prev, [item.id]: e.target.value }))} onKeyDown={e => e.key === 'Enter' && handleAddNote(item.id)} className="h-8 text-sm" />
                        ...
                        {notes.length > 0 ? (
                          <div className="space-y-1.5">
                            {notes.map((note: string, i: number) => (
                              <p key={i} className="text-xs text-muted-foreground bg-muted/50 rounded px-3 py-2">{note}</p>
                            ))}
                          </div>
                        ) : (
