...
export default function Reports() {
  ...
  useEffect(() => {
    Promise.all([getMyGeneratorRuns(), getMyValidationReports()]).then(([runs, reports]) => {
      ...
      setAllItems(items); setLoading(false);
    });
  }, []);

  const handleSaveIdea = async (name: string, source: string, score?: number) => {
    try { await addToBacklogDb({ ideaName: name, source, demandScore: score, status: 'New' }); toast.success(`"${name}" saved to My Ideas`); } catch { toast.error("Failed to save"); }
  };
              ...
              <Card className="rounded-2xl border-border/50 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="text-left min-w-0">
                        <p className="text-sm font-semibold truncate">
                          {item.type === 'run' ? `${item.data.persona} × ${item.data.category}` : (item.data.idea_text || "").slice(0, 60) + ((item.data.idea_text || "").length > 60 ? '...' : '')}
                        </p>
                        <p className="text-xs text-muted-foreground mt-0.5">{new Date(item.date).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <Badge variant="secondary" className="text-[10px] uppercase tracking-wider rounded-full">{item.type === 'run' ? 'Generator' : 'Validation'}</Badge>
                      {item.type === 'run' && <span className="text-xs text-muted-foreground font-medium">{Array.isArray(item.data.idea_suggestions) ? (item.data.idea_suggestions as any[]).length : 0} ideas</span>}
                ...
                <CollapsibleContent>
                  <div className="px-5 pb-5 border-t border-border/50 pt-4 space-y-4">
                    {item.type === 'run' && (
                      <>
                        <p className="text-xs font-medium text-muted-foreground">{Array.isArray(item.data.problem_clusters) ? (item.data.problem_clusters as any[]).length : 0} problem themes discovered</p>
                            ...
                            <Card key={i} className="rounded-xl bg-secondary/60 border-0">
                              <CardContent className="p-4 space-y-2">
                                <div className="flex items-start justify-between gap-2">
                                  ...
                                  <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={(e) => { e.stopPropagation(); handleSaveIdea(idea.name, 'Generated', idea.demandScore); }}><Bookmark className="h-3.5 w-3.5" /></Button>
                                </div>
                    ...
                    {item.type === 'report' && (
                      <div className="space-y-4">
                        ...
                        {item.data.mvp_wedge && (
                          <Card className="rounded-xl bg-secondary/60 border-0"><CardContent className="p-4">
                        ...
                        <Button variant="outline" size="sm" className="rounded-full" onClick={(e) => { e.stopPropagation(); handleSaveIdea(item.data.idea_text?.slice(0, 80), 'Validated'); }}>
